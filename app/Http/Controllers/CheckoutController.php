...
{
    //showing the checkout form, this is called second
    ...
    //validating the field display any errors and creating a order entry, this is called first.
    public function verifyCheckout(Request $request)
    {
        ...
        $shippingentry = ShippingEntry::create(['order_id'=>$order->order_id,'shipping_address'=>($request->input('address').' '.$request->input('city').' '.$request->input('zip')),'shipping_status'=>'PENDING','shipping_date'=>now()->format('Y-m-d H:i')]);
